...
int main(int argc, char *argv[]) {
    ...
    msgq_id = mq_open(MSGQOBJ_NAME, O_RDWR); // returns a file_descriptor which is our handle
                                             // to refer to the queue henceforth
    if (msgq_id == (mqd_t)-1) {
        perror("In mq_open()");
        exit(1);
    }
    ...
    /* getting a message: Blocks if there are no messages */
